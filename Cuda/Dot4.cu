...
__global__ void forward_kernel(
    ...
    // Get the warp id
    int warp_id = elem_idx / warpSize;

    // This creates a tree and stores the sum in every 32nd element
    for (int offset = warpSize/2; offset > 0; offset /= 2) {
        val += __shfl_down_sync_(0xFFFFFFFF, val, offset);
    }

    // Store in shared memory
    shared_mem[elem_idx] = val;

    // Wait for all threads to finish
    __syncthreads();

    // We then have to do a reduction on every 32nd element to get the final sum
    if (warp_id == 0) {
        T sum = val;

        // Sum all elements in the warp
        for (int i = warpSize; i < N; i+=warpSize) {
            sum += shared_mem[i];
        }
