    ...
    for (int A_idx = 0; A_idx < N; A_idx++) {
        ...
        for (int B_idx = 0; B_idx < M; B_idx++) {
            ...
            for (int d_idx = 0; d_idx < d; d_idx++) {
                ...
                sum += A[A_idx * d + d_idx] * B[B_idx * d + d_idx];
            }

            // Store the sum in the output matrix at output[A_idx, B_idx]
            output[A_idx * M + B_idx] = sum;
        }
    }
}
...
void forward(
    ...
    dim3 block(1); // Numebr of threads per "block" will be 1 for now.
    int shared_memory_size = 0; // No shared memory being used
