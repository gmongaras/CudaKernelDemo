...
__global__ void forward_kernel(
    ...
    T* shared_mem_A = shared_mem;
    T* shared_mem_B = &shared_mem[block_size_x * d];
    

    // Each thread can load some of A and some of B into shared memory
    for (int i = thread_idx_y; i < d; i += block_size_y) {
        shared_mem_A[thread_idx_x * d + i] = A[block_thread_idx_x * d + i];
    }
    for (int i = thread_idx_x; i < d; i += block_size_x) {
        shared_mem_B[thread_idx_y * d + i] = B[block_thread_idx_y * d + i];
    }
    ...
    for (int d_idx = 0; d_idx < d; d_idx++) {
        ...
        sum += shared_mem_A[thread_idx_x * d + d_idx] * shared_mem_B[thread_idx_y * d + d_idx];
    }
...
void forward(
    ...
    // We can allocate shared memory to store the A and B matrices
    // For A and B, we store a 32xd block in shared memory
    int shared_memory_size = 2 * 32 * d * sizeof(T);
